...
static char UIViewPreviousKeyboardFrame;
...
@property (nonatomic) CGRect previousKeyboardFrame;
...
- (void)inputKeyboardWillShow:(NSNotification *)notification {
	...
	BOOL constraintBasedKeyboardDidMoveBlockCalled = self.constraintBasedKeyboardDidMoveBlock && !CGRectIsNull(keyboardFrame);
	if (constraintBasedKeyboardDidMoveBlockCalled) {
		self.constraintBasedKeyboardDidMoveBlock(keyboardFrame, YES, NO);
	}
	...
	[UIView animateWithDuration:keyboardTransitionDuration
					 ...
					 animations:^{
						 ...
						 if (self.frameBasedKeyboardDidMoveBlock && !CGRectIsNull(keyboardFrame)) {
							 self.frameBasedKeyboardDidMoveBlock(keyboardFrame, YES, NO);
						 }
					 }
...
- (void)inputKeyboardWillHide:(NSNotification *)notification {
	...
	BOOL constraintBasedKeyboardDidMoveBlockCalled = self.constraintBasedKeyboardDidMoveBlock && !CGRectIsNull(keyboardFrame);
	if (constraintBasedKeyboardDidMoveBlockCalled) {
		self.constraintBasedKeyboardDidMoveBlock(keyboardFrame, NO, YES);
	}
	...
	[UIView animateWithDuration:keyboardTransitionDuration
					 ...
					 animations:^{
						 ...
						 if (self.frameBasedKeyboardDidMoveBlock && !CGRectIsNull(keyboardFrame)) {
							 self.frameBasedKeyboardDidMoveBlock(keyboardFrame, NO, YES);
						 }
					 }
					 completion:^(__unused BOOL finished) {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(__unused NSDictionary *)change context:(__unused void *)context {
	if ([keyPath isEqualToString:@"frame"] && (object == self.keyboardActiveView)) {
		CGRect keyboardFrame = [[object valueForKeyPath:keyPath] CGRectValue];
		
		if (CGRectEqualToRect(keyboardFrame, self.previousKeyboardFrame)) {
			return;
		}
		
		if (!self.keyboardActiveView.hidden && !CGRectIsNull(keyboardFrame)) {
			if (self.frameBasedKeyboardDidMoveBlock) {
				self.frameBasedKeyboardDidMoveBlock(keyboardFrame, NO, NO);
			}
			if (self.constraintBasedKeyboardDidMoveBlock) {
				self.constraintBasedKeyboardDidMoveBlock(keyboardFrame, NO, NO);
				[self layoutIfNeeded];
			}
		}
		
		self.previousKeyboardFrame = keyboardFrame;
	}
}
...
- (CGRect)previousKeyboardFrame {
	id previousFrameValue = objc_getAssociatedObject(self, &UIViewPreviousKeyboardFrame);
	if (previousFrameValue) {
		return [previousFrameValue CGRectValue];
	}
	return CGRectZero;
}

- (void)setPreviousKeyboardFrame:(CGRect)previousKeyboardFrame {
	[self willChangeValueForKey:@"previousKeyboardFrame"];
	objc_setAssociatedObject(self, &UIViewPreviousKeyboardFrame, [NSValue valueWithCGRect:previousKeyboardFrame], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
	[self didChangeValueForKey:@"previousKeyboardFrame"];
}
...
- (BOOL)isPanning {
	NSNumber *isPanningNumber = objc_getAssociatedObject(self, &UIViewIsPanning);
...
- (BOOL)keyboardWillRecede {
	...
	return (touchLocationInKeyboardWindow.y >= thresholdHeight) && (velocity.y >= 0.0f);
}
